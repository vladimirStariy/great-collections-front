    ...
    return <>
        <div className='w-full'>
            <Card className='flex flex-col gap-4 w-full max-w-3xl p-4 rounded-lg bg-base-200'>
                ...
                <CardBody className=" md:max-h-96 overflow-y-auto py-2">
                    <div className='flex flex-col gap-6 md:gap-4 w-full max-w-3xl'>
                        {props.customFields && props.customFields.map((item, index) => (<>
                            <div className='w-full flex flex-col gap-2'>
                                Field {index+1}
                                <div key={index} className='flex w-full flex-col md:flex-row gap-2 md:gap-4 items-center justify-between'>
                                    <Input 
                                    ...
                                    <Button color='danger' className='h-14 w-full md:w-auto' disabled={props.isLoading} onClick={() => props.removeDataField(index)}>
                                        <TrashIcon />
                                    </Button>
                                </div>
                            </div>
